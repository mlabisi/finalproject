    ...
    public void lookInDirection(int direction) {
        ...
        switch (direction) {
            ...
            case 1: //Left
                ...
                break;
            case 2: //Down
                if (!(playerY + 1 >= grid.length) && grid[playerY + 1][playerX].getType().compareToIgnoreCase("Room") != 0) {
                    grid[playerY + 1][playerX].switchLights(true);
                    if (!(playerY + 2 >= grid.length) && grid[playerY + 2][playerX].getType().compareToIgnoreCase("Room") != 0) {
                        grid[playerY + 2][playerX].switchLights(true);
                    }
                }
                break;
            case 3: //Right
                if (!(playerX + 1 >= grid.length) && grid[playerY][playerX + 1].getType().compareToIgnoreCase("Room") != 0) {
                    grid[playerY][playerX + 1].switchLights(true);
                    if (!(playerX + 2 >= grid.length) && grid[playerY][playerX + 2].getType().compareToIgnoreCase("Room") != 0) {
                        grid[playerY][playerX + 2].switchLights(true);
                    }
                }
                break;
